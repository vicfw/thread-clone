...
const FollowerCard = () => {
  ...
  const {
    followers: followersParams,
    item: ItemParams,
    following: followingParams,
  } = params;
  const followers = JSON.parse(followersParams as string);
  const item = JSON.parse(ItemParams as string);
  const following = JSON.parse(followingParams as string);
  ...
  return (
    <SafeAreaView>
      ...
      {active === 2 && (
        <Text className="text-[18px] text-center pt-10 text-black">
          No Pending
        </Text>
      )}
    </SafeAreaView>
